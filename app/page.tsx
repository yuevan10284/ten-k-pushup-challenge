...
export default function Home() {
  ...
  async function join(e:React.FormEvent) {e.preventDefault();setBusy(true);setMessage("");try{const data=await request({action:"join",name,pin});const next={id:data.participant.id,name:data.participant.name,pin};localStorage.setItem("pushup-profile",JSON.stringify(next));setProfile(next);setLogs(data.logs||[]);setStartedAt(data.participant.startedAt??null);setJoinOpen(false);refresh();}catch(e){setMessage(e instanceof Error?e.message:"Could not join.");}finally{setBusy(false);}}
  // A device can stay signed in as whoever last joined on it — there's no
  // expiry on the cached profile. The "not you?" escape hatch below covers
  // switching to a different person entirely; requiring the PIN fresh on
  // every save (never read from the cached profile) is what makes it
  // impossible to save reps as someone else without knowing their PIN.
  function switchAccount() {localStorage.removeItem("pushup-profile");setProfile(null);setLogs([]);setStartedAt(null);setName("");setPin("");setLogPin("");setMessage("");setJoinOpen(true);}
  async function addPushups(e:React.FormEvent) {e.preventDefault();if(!profile){setJoinOpen(true);return;}if(!/^\d{4}$/.test(logPin)){setMessage("Enter your 4-digit PIN to save.");return;}setBusy(true);setMessage("");try{const data=await request({action:"log",participantId:profile.id,pin:logPin,day,count:addMode?(logMap.get(day)||0)+count:count});const savedTotal=data.logs.reduce((sum:number,l:DayLog)=>sum+l.count,0);const crossed=[1000,2500,5000,7500,10000].filter(n=>total<n&&savedTotal>=n);if(crossed.length)setCelebrating(crossed);setLogs(data.logs);setMessage(`Day ${day} saved.`);if(addMode)setCount(0);setLogPin("");refresh();}catch(e){setMessage(e instanceof Error?e.message:"Could not save.");}finally{setBusy(false);}}
  ...
  return <main className="app-shell min-h-screen text-[#f4f3ef]">
    <div className="relative z-10 mx-auto max-w-[1440px] px-4 py-5 sm:px-7 sm:py-8 lg:px-10">
      ...
      <section className="dashboard-primary mb-7 grid gap-4 lg:grid-cols-[1.55fr_.75fr]">
        ...
        <form id="log-form" onSubmit={addPushups} className="glass-card log-panel rounded-[2rem] p-6 sm:p-8"><div className="mb-6 flex items-center justify-between"><div><p className="text-sm font-semibold uppercase tracking-[.16em] text-[#cfff57]">Log work</p><h2 className="font-display mt-1 text-3xl font-bold">Get your reps in.</h2><p className="mt-1 text-sm text-white/40">Small steps. Big results.</p></div><div className="icon-orb"><Plus /></div></div>{profile && <div className="mb-4 flex items-center justify-between rounded-xl bg-white/5 px-3 py-2 text-xs text-white/50"><span>Logging as <b className="text-white/80">{profile.name}</b></span><button type="button" onClick={switchAccount} className="font-semibold text-[#cfff57] hover:underline">Not you? Switch</button></div>}<label className="mb-2 block text-sm text-white/55">Challenge day</label><div className="relative"><CalendarDays className="select-icon" size={18}/><select value={day} onChange={(e) => { const d=Number(e.target.value); setDay(d); setCount(addMode?0:(logMap.get(d) ?? 0)); }} className="glass-input mb-4 h-13 w-full rounded-2xl pl-12 pr-4 text-base text-white outline-none">{plan.map((t,i)=><option className="bg-[#141b1d]" key={i} value={i+1}>Day {i+1} · target {t}</option>)}</select></div><label className="mb-2 block text-sm text-white/55">{addMode ? "Pushups completed" : "Day total (replaces saved total)"}</label><div className="flex flex-wrap gap-2 mb-3">{[10,25,50].map(n=><Button type="button" key={n} className="glass-button" onClick={()=>setCount(v=>Math.min(2000,v+n))}>+{n}</Button>)}<Button type="button" className="glass-button" onClick={()=>{setAddMode(!addMode);setCount(addMode?(logMap.get(day)||0):0)}}>{addMode?"Edit total":"Add reps"}</Button></div><div className="relative mb-4"><Dumbbell className="absolute left-4 top-1/2 z-10 -translate-y-1/2 text-white/45" size={18}/><Input type="number" min={0} max={2000} value={count} onChange={(e)=>setCount(Number(e.target.value))} className="glass-input h-13 rounded-2xl pl-12 text-lg text-white" /></div>{profile && <><label className="mb-2 block text-sm text-white/55">Your 4-digit PIN</label><div className="relative mb-4"><LockKeyhole className="absolute left-4 top-1/2 z-10 -translate-y-1/2 text-white/45" size={18}/><Input type="password" inputMode="numeric" value={logPin} onChange={(e)=>setLogPin(e.target.value.replace(/\D/g,"").slice(0,4))} placeholder="••••" className="glass-input h-13 rounded-2xl pl-12 text-xl tracking-[.4em] text-white" /></div></>}<Button disabled={busy} className="lime-button h-13 w-full rounded-2xl font-bold text-[#091009]">{busy ? "Saving…" : addMode ? "Update day" : "Replace day total"}<ArrowRight size={18}/></Button>{message && <p className="mt-3 text-sm text-[#ff886f]">{message}</p>}<p className="mt-4 text-center text-xs text-white/35">Tap any day below to add or edit it.</p></form>
      </section>
