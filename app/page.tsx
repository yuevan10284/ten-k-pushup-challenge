  ...
  return <main className="app-shell min-h-screen text-[#f4f3ef]">
    <div className="relative z-10 mx-auto max-w-[1440px] px-4 py-5 sm:px-7 sm:py-8 lg:px-10">
      ...
      <section className="grid gap-7 lg:grid-cols-[1.55fr_.75fr]">
        <div className="glass-card rounded-[2rem] p-5 sm:p-8"><div className="mb-6 flex items-end justify-between"><div><p className="text-sm text-white/45">Your 30-day plan</p><h2 className="font-display text-3xl font-bold">Built to ramp.</h2></div><span className="glass-chip text-sm text-white/55">Next: Day {nextDay}<ArrowRight size={15}/></span></div><div className="grid grid-cols-5 gap-2 sm:grid-cols-6 md:grid-cols-10">{plan.map((target,i)=>{ const actual=logMap.get(i+1); const done=actual!==undefined; const hit=done && actual>=target; return <button key={i} onClick={()=>selectDay(i)} aria-label={`Day ${i+1}, target ${target}`} className={`day-tile relative aspect-square p-1 text-center ${hit?"day-hit":done?"day-under":""}`}>{hit&&<span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#15200c] text-[#dfff73]"><Check size={13}/></span>}<span className="block text-xs opacity-55">{i+1}</span><span className="block text-sm font-bold sm:text-base">{actual ?? target}</span></button>})}</div><div className="mt-6 flex flex-wrap gap-5 text-xs text-white/40"><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-[#baff3b] shadow-[0_0_10px_#baff3b]"/> Target hit</span><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-[#ff896f]"/> Under target</span><span>Unlogged days re-ramp to your pace</span></div></div>
    ...
    <Athlete id={athleteId} onClose={()=>setAthleteId(null)} baseTargets={BASE_TARGETS}/>
