...
type DayLog = { day: number; count: number };
type Leader = { id: number; name: string; total: number; daysLogged: number; streak: number };
const TARGETS = [100,120,150,190,210,250,270,210,290,310,310,190,330,350,370,210,410,430,450,210,450,470,490,190,520,550,570,230,590,580];
...
export default function Home() {
  ...
  const nextDay = TARGETS.findIndex((t,i)=>(logMap.get(i+1)??0)<t)+1 || 30;
  const latestDay = logs.length ? Math.max(...logs.map((l) => l.day)) : 0;
  const targetToDate = TARGETS.slice(0, latestDay).reduce((sum, value) => sum + value, 0);
  ...
  const chartData = TARGETS.map((target, index) => {
    ...
    const targetTotal = TARGETS.slice(0, throughDay).reduce((sum, value) => sum + value, 0);
  ...
  useEffect(()=>{refresh();try{const saved=localStorage.getItem("pushup-profile");if(saved){const p=JSON.parse(saved);setProfile(p);loadLogs(p.id,p.pin);}}catch{setMessage("Please sign in again.");}const timer=setInterval(refresh,30000);return()=>clearInterval(timer);},[]);
  async function loadLogs(id:number,savedPin:string) {try{const r=await fetch(`/api/challenge?participant=${id}&pin=${encodeURIComponent(savedPin)}`,{cache:"no-store"});const d=await r.json();if(!r.ok)throw new Error(d.error);setLogs(d.logs);}catch{setMessage("Could not load your logs. Sign in again to retry.");}}
  async function join(e:React.FormEvent) {e.preventDefault();setBusy(true);setMessage("");try{const data=await request({action:"join",name,pin});const next={id:data.participant.id,name:data.participant.name,pin};localStorage.setItem("pushup-profile",JSON.stringify(next));setProfile(next);setLogs(data.logs||[]);setJoinOpen(false);refresh();}catch(e){setMessage(e instanceof Error?e.message:"Could not join.");}finally{setBusy(false);}}
  ...
  return <main className="app-shell min-h-screen text-[#f4f3ef]">
    <div className="relative z-10 mx-auto max-w-[1440px] px-4 py-5 sm:px-7 sm:py-8 lg:px-10">
      ...
      <section className="mb-7 grid gap-4 lg:grid-cols-[1.55fr_.75fr]">
        <div className="glass-card hero-panel relative overflow-hidden rounded-[2rem] p-6 sm:p-9"><div className="relative z-10"><div className="flex items-start justify-between gap-6"><div><div className="lime-pill mb-7 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-[#dfff73]"><Flame size={17}/> The 10,000 Club</div><p className="number-glow font-display text-[clamp(4.5rem,11vw,7.8rem)] font-black leading-[.76] tracking-[-.07em]">{total.toLocaleString()}</p><p className="mt-5 text-lg text-white/55">of 10,000 pushups completed</p></div><Motivation/></div><div className="mt-7 flex items-center gap-4"><Progress value={Math.min(100, total / 100)} className="h-3 bg-white/10 [&>div]:bg-gradient-to-r [&>div]:from-[#c8ff42] [&>div]:to-[#8eff1f] [&>div]:shadow-[0_0_18px_#aaff36]" /><b className="text-sm">{Math.round(total / 100)}%</b></div><div className="mt-7 grid grid-cols-3 divide-x divide-white/10"><Metric icon={<Target/>} value={`${Math.round(total/100)}%`} label="complete"/><Metric icon={<Dumbbell/>} value={Math.max(0,10000-total).toLocaleString()} label="remaining"/><Metric icon={<CalendarDays/>} value={`${logs.length} day${logs.length===1?"":"s"}`} label="logged"/></div></div></div>
        <form id="log-form" onSubmit={addPushups} className="glass-card rounded-[2rem] p-6 sm:p-8"><div className="mb-6 flex items-center justify-between"><div><p className="text-sm font-semibold uppercase tracking-[.16em] text-[#cfff57]">Log work</p><h2 className="font-display mt-1 text-3xl font-bold">Get your reps in.</h2><p className="mt-1 text-sm text-white/40">Small steps. Big results.</p></div><div className="icon-orb"><Plus /></div></div><label className="mb-2 block text-sm text-white/55">Challenge day</label><select value={day} onChange={(e) => { const d=Number(e.target.value); setDay(d); setCount(addMode?0:(logMap.get(d) ?? 0)); }} className="glass-input mb-4 h-13 w-full rounded-2xl px-4 text-base text-white outline-none">{TARGETS.map((_,i)=><option className="bg-[#141b1d]" key={i} value={i+1}>Day {i+1} · target {TARGETS[i]}</option>)}</select><label className="mb-2 block text-sm text-white/55">{addMode ? "Reps to add" : "Day total (replaces saved total)"}</label><div className="flex flex-wrap gap-2 mb-3">{[10,25,50].map(n=><Button type="button" key={n} className="glass-button" onClick={()=>setCount(v=>Math.min(2000,v+n))}>+{n}</Button>)}<Button type="button" className="glass-button" onClick={()=>{setAddMode(!addMode);setCount(addMode?(logMap.get(day)||0):0)}}>{addMode?"Edit total":"Add reps"}</Button></div><div className="relative mb-4"><Dumbbell className="absolute left-4 top-1/2 z-10 -translate-y-1/2 text-white/45" size={18}/><Input type="number" min={0} max={2000} value={count} onChange={(e)=>setCount(Number(e.target.value))} className="glass-input h-13 rounded-2xl pl-12 text-lg text-white" /></div><Button disabled={busy} className="lime-button h-13 w-full rounded-2xl font-bold text-[#091009]">{busy ? "Saving…" : addMode ? "Save reps" : "Replace day total"}<ArrowRight size={18}/></Button>{message && <p className="mt-3 text-sm text-[#ff886f]">{message}</p>}<p className="mt-4 text-center text-xs text-white/35">Day {day}: {logMap.get(day)||0} / {TARGETS[day-1]} saved reps</p></form>
      </section>
      <section className="grid gap-7 lg:grid-cols-[1.55fr_.75fr]">
        <div className="glass-card rounded-[2rem] p-5 sm:p-8"><div className="mb-6 flex items-end justify-between"><div><p className="text-sm text-white/45">Your 30-day plan</p><h2 className="font-display text-3xl font-bold">Built to ramp.</h2></div><span className="glass-chip text-sm text-white/55">Next: Day {nextDay}<ArrowRight size={15}/></span></div><div className="grid grid-cols-5 gap-2 sm:grid-cols-6 md:grid-cols-10">{TARGETS.map((target,i)=>{ const actual=logMap.get(i+1); const done=actual!==undefined; const hit=done && actual>=target; return <button key={i} onClick={()=>selectDay(i)} aria-label={`Day ${i+1}, target ${target}`} className={`day-tile relative aspect-square p-1 text-center ${hit?"day-hit":done?"day-under":""}`}>{hit&&<span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[#15200c] text-[#dfff73]"><Check size={13}/></span>}<span className="block text-xs opacity-55">{i+1}</span><span className="block text-sm font-bold sm:text-base">{actual ?? target}</span></button>})}</div><div className="mt-6 flex flex-wrap gap-5 text-xs text-white/40"><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-[#baff3b] shadow-[0_0_10px_#baff3b]"/> Target hit</span><span><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full bg-[#ff896f]"/> Under target</span><span>Numbers show target until logged</span></div></div>
        <div className="glass-card relative overflow-hidden rounded-[2rem] p-5 sm:p-8"><div className="relative z-10 mb-6 flex items-center justify-between"><div><p className="text-sm text-white/45">Everybody eats</p><h2 className="font-display text-3xl font-bold">Leaderboard</h2></div><Trophy className="text-[#bdff43] drop-shadow-[0_0_12px_#9dff22]" /></div><Input aria-label="Find participant" placeholder="Find someone in the crew…" value={search} onChange={e=>setSearch(e.target.value)} className="glass-input mb-4"/>{boardError&&<button onClick={refresh} className="text-sm text-amber-200">{boardError}</button>}{leaders.length ? <div className="relative z-10 space-y-2">{leaders.filter(l=>l.name.toLowerCase().includes(search.toLowerCase())).map((leader,i)=><div key={leader.id} role="button" tabIndex={0} aria-label={`View ${leader.name} progress`} onClick={()=>setAthleteId(leader.id)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setAthleteId(leader.id)}}} className={`leader-row cursor-pointer flex items-center gap-3 rounded-2xl p-3 ${profile?.id===leader.id?"leader-me":""}`}><span className={`grid h-9 w-9 place-items-center rounded-full text-sm font-bold ${i===0?"rank-one":"bg-white/8 text-white/55"}`}>{i+1}</span><div className="min-w-0 flex-1"><p className="truncate font-semibold">{leader.name}</p><p className="text-xs text-white/35">{leader.daysLogged} days · {leader.streak} streak</p></div><b>{leader.total.toLocaleString()}</b></div>)}</div>:<div className="relative z-10 grid min-h-52 place-items-center text-center text-white/40"><div><Users className="mx-auto mb-3"/><p>Be first on the board.</p></div></div>}<p className="mt-5 text-sm text-slate-300">Tap a name to view their calendar and chart.</p></div>
      </section>
    ...
    <Athlete id={athleteId} onClose={()=>setAthleteId(null)} targets={TARGETS}/>
